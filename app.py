...
```"""
    ...
    # 4. MASTER VISUALIZATION SUITE
    st.markdown("---")
    st.header("📊 4. Connected Master Visualization Suite")

    # [Code for charts remains unchanged here to save visual clarity; handles 7 basic renders based on input schema]
    if len(categorical_cols) > 0 and len(numeric_cols) > 0:
        ...
        fig1 = px.bar(active_df, x=c_x, y=c_y, title=f"Total {c_y} by {c_x}", template="plotly_white")
    ...
    def compile_granular_pdf():
        ...
        h2_style = ParagraphStyle('SectionH2', parent=styles['Heading3'], fontSize=12, spaceBefore=10, spaceAfter=6, textColor=colors.HexColor('#374151'))
        ...
        # SECTION 3: NUMERICAL PARAMETRIC PROFILES (LOOP ALL NUMERIC COLUMNS)
        ...
        for col in categorical_cols[:8]:  # Top 8 columns to keep layout tight and cleanly formatted
        ...
        # Build document and close buffer channel
        doc.build(story)
        buffer.seek(0)
